...
class Partnerships(commands.Cog):
    ...
    async def get_server_name(self, invite_code):
    ...
    async def get_user_stats(self, user_id, period="all"):
        ...
        for i, timestamp in enumerate(user_data["timestamps"]):
            ...
            server_name = await self.get_server_name(user_data["servers"][i])
            filtered["server_names"].append(server_name)

        return filtered

    async def get_all_stats(self, period="all"):
        now = datetime.now(timezone.utc)
        stats = {}
    ...
    async def pm_info(self, ctx, member: nextcord.Member = None):
        ...
        stats = await self.get_user_stats(member.id, "all")
    ...
    async def pm_servers(self, ctx):
        ...
        for invite_code, count in sorted_servers[:15]:
            name = await self.get_server_name(invite_code)
